...
> 1. 约瑟夫环 LinkedList;li.add(i);(index = (index + m),%li.size();li.remove(index-\-);) li.get(0); ， ✔️
> 2. [数组排成最小的数](https://www.weiweiblog.cn/printminnumber/) Arrays.sort(str, new Comparator<String>(){ public int compare(String s1, String s2)，✔️
> 3. 最小的K个数(堆排序)， [数据流中的中位数 （2 PriorityQueue）](https://www.weiweiblog.cn/getmedian/).   
> 4. **最小的K个数** （2fun）快排思想 part return l, 外else return left; void set_k(int\* input, n, k) ， ✔️
> 5. **quickSort**(a[],left,right), while(1) { (双while, if(l >= r) break; swap) } swap(a[left], a[l]);， ✔️
> 6. mergeSort (2 mergeSort + new \*arr + 3 while + 1 for)
